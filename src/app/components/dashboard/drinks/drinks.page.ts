...
import { ALERT_BTNS, ALERT_HEADERS } from 'src/app/utilities/alertModal';
  ...
  export class DrinksPage implements OnInit {
    ...
    pathLoad: string = API_PATHS.drinks
    ...
    fields: string[] = ['index', 'name', 'DrinkType.type', 'price', 'description', 'image']
    money: string[] = ['price']
    ...
    loadTypes() {
      this.restApi.get(API_PATHS.drinkTypes+'list').subscribe((response:any)=>{
